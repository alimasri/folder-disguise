class GUIDList:
    ...
    @classmethod
    def read_guids(cls):
        guids_list = [line.strip().split(';') for line in LIST.split("\n")]
        return guids_list

    @staticmethod
    def print_options(guids_list):
        if guids_list is None:
            return
        for i in range(0, len(guids_list), 2):
            s1 = "[{}] {}".format(i, guids_list[i][0])
            s2 = ''
            if i + 1 != len(guids_list):
                s2 = "[{}] {}".format(i + 1, guids_list[i + 1][0])
            print("{0:60} {1}".format(s1, s2))


LIST = '''Add Network Location;{D4480A50-BA28-11d1-8E75-00C04FA31A86}
